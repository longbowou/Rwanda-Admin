        ...
        <div class="card card-custom gutter-b">
          <div class="card-header">
            <div class="card-title">
              ...
              <h3 class="card-label">{{ $t("Add a Service Category") }}</h3>
            </div>
            <div class="card-toolbar">
              <button
                @click="$router.push({ name: 'services-categories' })"
                class="btn btn-light-dark font-weight-bolder mr-2"
              >
                <i class="ki ki-long-arrow-back icon-lg"></i>
                {{ $t("Back To Service Categories") }}
              </button>
            </div>
          </div>
          <div class="card-body">
...
export default {
  ...
  mounted() {
    this.$store.dispatch(SET_BREADCRUMB, [
      { title: this.$t("Add a Service Category") }
    ]);
    this.$store.dispatch(SET_HEAD_TITLE, this.$t("Add a Service Category"));
  },
  methods: {}
};
</script>
